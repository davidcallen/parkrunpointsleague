...
module "dns" {
  ...
  route53_testing_mode_ami_id        = (length(data.aws_ami.centos-7) > 0) ? data.aws_ami.centos-7.id : ""
...
output "route53_private_hosted_zone_id" {
  value = (module.global_variables.route53_enabled) ? module.dns[0].route53_private_hosted_zone_id : ""
}
output "route53_endpoint_inbound_ips" {
  value = (module.global_variables.route53_enabled) ? module.dns[0].route53_endpoint_inbound_ips : []
}
output "route53_endpoint_outbound_ips" {
  value = (module.global_variables.route53_enabled) ? module.dns[0].route53_endpoint_outbound_ips : []
}
output "route53_ec2_test_ip_address" {
  value = (module.global_variables.route53_enabled) ? module.dns[0].ec2_test_ip_address : ""
}
...
#
# VERDICT:  NOT WORKING
#   - aws_route53_vpc_association_authorization fails from Core to backbone PHZ  (how to solve?)
#   - route53 and simpleAD are just not compatible in a cross-account !
#     - If use AD for DHCP DNS setting then bad EFS resolution
#     - How to get Endpoints to forward to our AD (cant from Inbound endpoint) ?
# ---------------------------------------------------------------------------------------------------------------------
//module "dns" {
//  count                              = (module.global_variables.route53_enabled) ? 1 : 0
...
//  route53_use_endpoints              = module.global_variables.route53_use_endpoints
//  route53_testing_mode_enabled       = var.route53_testing_mode_enabled
//  route53_testing_mode_ami_id        = data.aws_ami.centos-7.id
//  route53_direct_dns_update_enabled  = module.global_variables.route53_direct_dns_update_enabled
//  environment                        = var.environment
//  vpc = {
//    vpc_id                      = module.vpc.vpc_id
...
//  }
//  share_with_account_ids    = var.cross_account_access.accounts[*].account_id
//  central_directory_enabled = module.global_variables.central_directory_enabled
//
//  # HACK HACK HACK HACK HACK HACK HACK HACK - temporary
//  central_directory_admin_password                             = "6TNYpgSHySPXkg7T3QR2KJw2aP0N41" # local.secrets_primary.data["ad-admin.password"]
