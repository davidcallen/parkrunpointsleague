...
terraform {
  backend "s3" {
    ...
    #bucket         = "${module.global_variables.org_domain_name}-${module.global_variables.org_short_name}-terraform-state-prod"
    bucket         = "parkrunpointsleague.org-prpl-prod-terraform-state" # This S3 bucket is provisioned in our bootstrap module
    encrypt        = true
    key            = "terraform.tfstate"
    dynamodb_table = "prpl-prod-terraform-state-locking" # This DynamoDB table is provisioned in our bootstrap module
