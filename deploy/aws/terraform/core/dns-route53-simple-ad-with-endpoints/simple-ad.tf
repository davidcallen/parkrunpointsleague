...
//  enable_sso = false
//  #  name     = "${var.environment.resource_name_prefix}-simple-ad"
//  type     = "SimpleAD"
...
//}
//# Output our above SimpleAD IPs so this can be read by the backbone terraform when associating the DNS to ClientVPN
...
locals {
  central_directory_dns_ips_str = (var.central_directory_enabled && length(data.local_file.central_directory_dns_ips) > 0) ? data.local_file.central_directory_dns_ips["core/outputs/terraform-output-central-directory-dns-ips"].content : ""
...
locals {
  ...
  central_directory_dns_ip_cidrs = [for ip in local.central_directory_dns_ips : "${ip}/32"]
}
module "ad-security-group-for-linux" {
  ...
  name        = "ad-for-linux"
  ad_ip_cidrs = local.central_directory_dns_ip_cidrs # "${var.eu-west-1_active_directory_ip_addresses[*]}/32"
}
output "central_directory_dns_server_ips" {
  value = local.central_directory_dns_ip_cidrs
}