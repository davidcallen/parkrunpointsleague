...
module "simple-ad-admin-win" {
  ...
  name_suffix                       = ""
  hostname_fqdn                     = (var.org_using_subdomains) ? "${var.environment.resource_name_prefix}-simple-ad-admin-win.${var.environment.name}.${var.org_domain_name}" : "${var.environment.resource_name_prefix}-simple-ad-admin-win.${var.org_domain_name}"
