...
//  key_id            = "arn:aws:kms:${module.global_variables.aws_region}:${module.global_variables.core_account_id}:key/3e3abac5-de32-460b-8453-6e7de2ea0fa6" #  aws_kms_key.a.key_id
//  grantee_principal = "arn:aws:iam::${var.environment.account_id}:role/aws-service-role/autoscaling.amazonaws.com/AWSServiceRoleForAutoScaling" # aws_iam_role.a.arn
//  operations        = [
//    "Encrypt",
//    "Decrypt",
//    "ReEncryptFrom",
...
# ---------------------------------------------------------------------------------------------------------------------
# AMI for our CentOS 7 Jenkins Controller (inherit from our base CentOS 7 with installs (jenkins)
# ---------------------------------------------------------------------------------------------------------------------
# aws --region eu-west-1 ec2 describe-images --owners self --filters Name=product-code,Values=aw0evgkw8e5c1q413zgy5pjce --filters Name=name,Values=prpl-jenkins-*
data "aws_ami" "centos-7-jenkins-controller" {
  count       = var.amis.centos7_jenkins_controller.enabled ? 1 : 0
  most_recent = true
  # centos7 based
  filter {
    name   = "product-code"
  ...
  filter {
    name   = "block-device-mapping.encrypted"
    values = [var.amis.centos7_jenkins_controller.use_encrypted]
  }
  owners = ["self"]
}
...
# ---------------------------------------------------------------------------------------------------------------------
# aws --region eu-west-1 ec2 describe-images --owners self --filters Name=product-code,Values=aw0evgkw8e5c1q413zgy5pjce --filters Name=name,Values=prpl-centos-7-nexus-*
data "aws_ami" "centos-7-nexus" {
  count       = var.amis.centos7_nexus.enabled ? 1 : 0
  most_recent = true
  # centos7 based
  filter {
    name   = "product-code"
