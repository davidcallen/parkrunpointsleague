...
{
    echo  "+----------------------------------------------------------------------+"
    echo  "| create-schema.sh - Create database Schema                            |"
    echo  "+----------------------------------------------------------------------+"
    echo  ""
    ...
    echo  ""
    echo  " Examples"
    echo  "    ./create-schema.sh --host prpl-mysql -p XXXXXXXXX -k"
    echo  ""
    exit 1
}
...
if [ "${ARG_USE_KUBECTL_RUN}" == "TRUE" ] ; then
	...
	cat create-schema.sql | kubectl run --stdin=true --rm --image=mysql:5.7 --restart=Never mysql-client -- mysql -h ${ARG_MYSQL_HOST} -u PRPL -p${ARG_PRPL_PWD} -B --
else
